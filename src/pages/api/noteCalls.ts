async function fetchAllNotes() {
  try {
    const res = await fetch('http://localhost:8080/api/v1/notes/get')
    if (!res.ok) {
...
async function postNote(note: object) {
  try {
    const res = await fetch('http://localhost:8080/api/v1/notes/create', {
      method: 'POST', 
...
async function deletePost(noteId: string) {
  try {
    const res = await fetch(`http://localhost:8080/api/v1/notes/delete/${noteId}`, {
      method: 'DELETE',
      headers: {'Content-Type': 'application/json'}
    })
    if (!res.ok) {
...
async function patchNote(noteId: string, noteToUpdate: object) {
  try {
    const res = await fetch(`http://localhost:8080/api/v1/notes/update/${noteId}`, {
      method: "PATCH",
      headers: {'Content-Type': 'application/json'},
