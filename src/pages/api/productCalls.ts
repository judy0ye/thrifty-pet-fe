const postProduct = async (productUrl: string) => {
  try {
    const res = await fetch(`https://thrifty-pet-be-4c9efd8c099d.herokuapp.com/api/v1/products/create`, {
      method: 'POST',
...
const getAllProducts = async () => {
  try {
    const res = await fetch('https://thrifty-pet-be-4c9efd8c099d.herokuapp.com/api/v1/products/get')
    if (!res.ok) {
...
const getProductById = async (productId: string) => {
  try {
    const res = await fetch(`https://thrifty-pet-be-4c9efd8c099d.herokuapp.com/api/v1/products/get/${productId}`)
    if (!res.ok) {
